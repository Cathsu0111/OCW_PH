...
<script>
    ...
    function GetPaymentMethod() {
        PaymentClient.GetPaymentMethodByCategory(WebInfo.SID, Math.uuid(), "GASH", 0, function (success, o) {
            if (success) {
                if (o.Result == 0) {
    ...
    function ConfirmPayPalDeposit() {
        PaymentClient.ConfirmEPayDeposit(WebInfo.SID, Math.uuid(), OrderNumber, ActivityNames, lang,"GASH",1,function (success, o) {
            window.parent.API_LoadingEnd(1);
             if (success) {
                 if (o.Result == 0) {
                     window.parent.showMessageOK(mlp.getLanguageKey("成功"), mlp.getLanguageKey("前往付款"), function () {
                        var data = o.Data;
                        window.open(`/Payment/EPay/EPAYSendPayment.aspx?amount=${data.Amount}&paymentCode=${data.PaymentCode}&webSID=${WebInfo.SID}&orderNumber=${data.PaymentSerial}&UserName=${data.ToInfo}&Type=GASH`, "_blank");
                    });
                        ...
                        <div class="card-item sd-03">
                            <div class="card-item-link">
                                <div class="card-item-inner">
                                    <div class="title">
                                        <span class="language_replace">Lucky Fanta</span>
